...
final class EpisodeFeatureTest extends TestCase
{
    ...
    public function test_get_all_episodes_returns_200(): void
    {
        Episode::factory()->count(3)->create(['movie_id' => $this->serial->id]);

        $response = $this->actingAs($this->admin, 'api')
            ->getJson("/api/v1/movies/{$this->serial->id}/episodes");

        $response->assertOk()
            ->assertJsonCount(3, 'data');
    }

    public function test_get_all_episodes_returns_empty_when_none_exist(): void
    {
        $response = $this->actingAs($this->admin, 'api')
            ->getJson("/api/v1/movies/{$this->serial->id}/episodes");
    ...
    public function test_get_all_episodes_requires_authentication(): void
    {
    ...
    public function test_get_episode_by_id_returns_200(): void
    {
        $episode = Episode::factory()->create(['movie_id' => $this->serial->id]);

        $response = $this->actingAs($this->admin, 'api')
            ->getJson("/api/v1/movies/{$this->serial->id}/episodes/{$episode->id}");

        $response->assertOk()
            ->assertJsonFragment(['title' => $episode->title]);
    }

    public function test_get_episode_by_id_returns_404_when_not_found(): void
    {
        $response = $this->actingAs($this->admin, 'api')
            ->getJson("/api/v1/movies/{$this->serial->id}/episodes/999");

        $response->assertNotFound();
    }

    public function test_get_episode_by_id_returns_404_when_belongs_to_different_serial(): void
    {
    ...
    public function test_get_episode_by_id_requires_authentication(): void
    {
        $episode = Episode::factory()->create(['movie_id' => $this->serial->id]);
